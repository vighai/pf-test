...
function semplice_import_cover($post_id) {
	...
	if(null !== $ram && isset($ram['cover'])) {
		...
		if(isset($cover_styles['background-image'])) {
			$output['images'][$cover_styles['background-image']] = semplice_get_image_object($cover_styles['background-image'], 'full');
		}
		// video fallback
		if(isset($cover_styles['bg_video_fallback'])) {
			$output['images'][$cover_styles['bg_video_fallback']] = semplice_get_image_object($cover_styles['bg_video_fallback'], 'full');
		}
		// cover iterate
		$cover_iterate = array();
		// is old single row mode?
